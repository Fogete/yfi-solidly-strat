...
contract Strategy is BaseStrategy {
    ...
    // Swap stuff
    address internal constant solidlyRouter = 0xa38cd27185a464914D3046f0AB9d43356B34829D;
    bool public tradesEnabled;
    bool public realiseLosses;
    bool public depositerAvoid;
    address public tradeFactory = address(0xD3f89C21719Ec5961a3E6B0f9bBf9F9b4180E9e9);

    IERC20 internal constant wftm = IERC20(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    IAnyWFTM internal constant anyWFTM = IAnyWFTM(0x6362496Bef53458b20548a35A2101214Ee2BE3e0);
    ...
    address public lpToken = address(0x9aC7664060a3e388CEB157C5a0B6064BeFFAb9f2);
    ...
    // wftm and anywFTM are interchangeable 1-1. So we need our balance of each.
    // Added to whatever we can withdraw from lps.
        ...
        )
    {
        ...
        if (assets >= debt) {
            ...
            // Loss should never happen. so leave blank. Small potential for IP I suppose.
            // Let's not record if so and handle manually.
            // Don't withdraw either in case we realize losses.
            // Withdraw with loss.
            if (realiseLosses) {
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        // Don't bother for less than 0.1 wftm
        if (toInvest > 1e17) {
            // Because it is a stable pool, lets check slippage by doing a trade against it.
            // If we can swap 1 wftm for less than slippage we gucci.
            ...
            if (amountOut < inAmount.mul(lpSlippage).div(DENOMINATOR)) {
                // Don't do anything because we would be lping into the lp at a bad price
                return;
            }
